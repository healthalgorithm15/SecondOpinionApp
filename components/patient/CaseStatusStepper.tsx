...
export const CaseStatusStepper = ({ currentCase }: { currentCase: any }) => {
  ...
  return (
    <View style={styles.statusCard}>
      <Text style={styles.statusHeader}>{isCompleted ? "Analysis Complete" : "Active Case"}</Text>
      ...
      <View style={styles.stepperContainer}>
        ...
        <StepItem icon="robot-outline" label="AI" active={status === 'AI_PROCESSING'} completed={status !== 'AI_PROCESSING'} />
        <StepItem icon="doctor" label="Expert" active={status === 'PENDING_DOCTOR'} completed={isCompleted} />
        <StepItem icon="check-circle-outline" label="Final" active={isCompleted} completed={isCompleted} />
      </View>
      ...
      >
        <Text style={styles.detailsBtnText}>{isCompleted ? "View Results" : "Track Progress"}</Text>
      </TouchableOpacity>
    </View>
  );
};
...
const styles = StyleSheet.create({
  statusCard: { backgroundColor: 'white', marginHorizontal: 20, padding: 24, borderRadius: 28, ...SHADOWS.soft, borderWidth: 1, borderColor: '#E2E8F0', marginBottom: 20 },
  ...
  stepCircle: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#F1F5F9', justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  stepLabel: { fontSize: 10, color: '#94A3B8', textAlign: 'center' },
  detailsBtn: { backgroundColor: COLORS.primary, height: 56, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
